...
// Graph and Pie chart initialized
chart();
pie();
...
function math() {
  ...
  for (let o = 0; o < numBrackets; o++) {
    ...
    if (o == numBrackets - 1) {
      upto = topOne[topOne.length - 1].toString();
  ...
  for (let i = 0; i < 109; i++) {
    ...
    for (let j = numBrackets - 1; j > -1; j--) {
      ...
      income -= taxed;
      console.log("income in pos ", i, ":", income);
    }
...
function chartData1p() {
  ...
  if (checkbox.checked) {
    ...
    myChart1.data.labels = oneTo99.slice(0, 10);
  } else {
    chartXmax = 99;
    chartAllIncome = allIncome;
    chartIncomeAfter = IncomeAfter.slice(0, 99);
    myChart1.data.labels = oneTo99;
  }
}
...
function plus() {
  ...
  brackethtml.innerHTML =
    '<div class="band_input" id="band' +
    numBrackets +
    '">' +
    '<h3 class="subtitle">Tax Band ' +
    numBrackets +
    "</h3>" +
